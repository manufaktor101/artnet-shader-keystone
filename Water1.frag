precision mediump float;
  ...
  vec4 osc(vec2 _st, float frequency, float sync, float offset) {
         vec2 st = _st;
   float r = sin((st.x-offset/frequency+u_time*sync)*frequency)*0.5  + 0.5;
   float g = sin((st.x+u_time*sync)*frequency)*0.5 + 0.5;
   float b = sin((st.x+offset/frequency+u_time*sync)*frequency)*0.5  + 0.5;
  ...
  void main () {
    vec4 c = vec4(0.582,0.475,1.000,1.000);
    vec2 st = gl_FragCoord.xy/u_resolution.xy;
    gl_FragColor = color(osc(rotate(modulateRotate(st, shape(st, 999., 0.3, 0.5), 1.57, 0.), angle0, 0.), 3., -0.5, 0.5), 0., 0.3, 0.2, 1.);
  }